...
'---------------------------------------------------------------------------------------
' Date      : 8/31/05
' Purpose   : Show a browse for folder dialog.
'---------------------------------------------------------------------------------------
...
Const ValidFlags As Long = _
        ...
        BIF_VALIDATE Or _
        BIF_USENEWUI Or _
        BIF_BROWSEFORCOMPUTER Or _
        BIF_BROWSEFORPRINTER Or _
    ...
    lsTitle = StrConv(tFolderDialog.sTitle & vbNullChar, vbFromUnicode)             'store an ansi version of the title
    
    Dim lsInitPath As String
    If LenB(tFolderDialog.sInitialPath) Then
       lsInitPath = StrConv(tFolderDialog.sInitialPath & vbNullChar, vbFromUnicode) 'store an ansi version of the initial folder
    End If
    
    tBI.lParam = StrPtr(lsInitPath)                                                 'store a pointer to the initial folder, or 0 if none
    
    tBI.hWndOwner = tFolderDialog.hWndOwner                                         'store the hwndowner
    tBI.lpszTitle = StrPtr(lsTitle)                                                 'store the dialog title
    tBI.ulFlags = tFolderDialog.iFlags And ValidFlags                               'mask out invalid flags
    ...
    tBI.lpfn = pGetHook(CLng(tFolderDialog.iFlags), BIF_EnableHook, _
                        dlgTypeFolder, tFolderDialog.oHookCallback)                 'allocate a hook procedure
    
    If CBool(tFolderDialog.iFlags And BIF_USENEWUI) Then
        Static oCoInit As pcCoInitialize
        If oCoInit Is Nothing Then Set oCoInit = New pcCoInitialize
    End If
    
    tBI.iImage = ZeroL                                                              'not used
    tBI.pszDisplayName = ZeroL                                                      'not used
    
    pidlOut = SHBrowseForFolder(tBI)                                                'call the api
    pReleaseHook tBI.lpfn                                                           'release the hook procedure, if any
    ...
    If pidlOut Then
        ...
        pFolder_Free pidlOut                                                        'Free the pidl
        
    End If
    
    If pIDLRoot Then pFolder_Free pIDLRoot                                          'Free the pidl
    
End Function

Public Function Folder_GetSpecial(ByVal hWndOwner As Long, ByVal iFolder As Long) As String
'---------------------------------------------------------------------------------------
' Date      : 8/31/05
' Purpose   : Return the path to a special operating system folder
'---------------------------------------------------------------------------------------
    Dim pIdl As Long
    If SHGetSpecialFolderLocation(hWndOwner, iFolder, pIdl) = ZeroL Then 'get the pidl
        Folder_GetSpecial = Folder_PathFromPidl(pIdl)                    'Convert it to a path
        pFolder_Free pIdl                                                'Free the pidl
    End If
End Function
