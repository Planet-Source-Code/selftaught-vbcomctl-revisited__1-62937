...
'###############################################
    ...
    Public Function CreateFile(ByVal lpFileName As Long, ByVal dwDesiredAccess As Long, ByVal dwShareMode As Long, ByVal lpSecurityAttributes As Long, ByVal dwCreationDisposition As Long, ByVal dwFlagsAndAttributes As Long, ByVal hTemplateFile As Long) As Long
        CreateFile = vbComCtlTlb.CreateFile(ByVal lpFileName, dwDesiredAccess, dwShareMode, ByVal lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile)
        If CreateFile Then
            DEBUG_Add DEBUG_hFile, CreateFile
        End If
    End Function
    
    Public Function CloseHandle(ByVal hFile As Long) As Long
        CloseHandle = vbComCtlTlb.CloseHandle(hFile)
        If CloseHandle Then
            DEBUG_Remove DEBUG_hFile, hFile
        End If
    End Function
...
'###############################################
    ...
    Public Function SHGetSpecialFolderLocation(ByVal hWndOwner As Long, ByVal iFolder As Long, ByRef pIdl As Long) As Long
        SHGetSpecialFolderLocation = vbComCtlTlb.SHGetSpecialFolderLocation(hWndOwner, iFolder, pIdl)
        If SHGetSpecialFolderLocation = ZeroL Then
            DEBUG_Add DEBUG_pIdl, pIdl
        End If
    End Function
    
    Public Function SHBrowseForFolder(ByRef tBI As BROWSEINFO) As Long
        SHBrowseForFolder = vbComCtlTlb.SHBrowseForFolder(tBI)
        If SHBrowseForFolder Then
            DEBUG_Add DEBUG_pIdl, SHBrowseForFolder
        End If
    End Function
